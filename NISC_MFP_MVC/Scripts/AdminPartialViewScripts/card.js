  ...
  const columns = [
    ...
    { data: "enable", name: "使用狀態" },
    {
      ...
      render: function (data, type, row) {
        return (
          "<div class='row row-cols-sm-1 row-cols-md-1 row-cols-lg-1 row-cols-xl-3 row-cols-xxl-3 g-2'><div class='col'><button type='button' class='btn btn-primary btn-sm btn-deposit' data-id='" +
          data.serial +
          "'><i class='fa-solid fa-circle-info me-1'></i><div style='display: inline-block; white-space: nowrap;'>儲值</div></button></div>" +
...
function ColumnSearch() {
  ...
  $("#searchCard_AttributeSelect").change(function () {
    if ($("#searchCard_AttributeSelect").val() != "") {
      dataTable
        .columns(5)
        .search($("#searchCard_AttributeSelect :selected").text())
        .draw();
    } else {
  ...
  $("#searchCard_StatusSelect").change(function () {
    if ($("#searchCard_StatusSelect").val() != "") {
      dataTable
        .columns(6)
        .search($("#searchCard_StatusSelect :selected").text())
        .draw();
    } else {
...
function DeleteAlertPopUp() {
  ...
  const url = "/Admin/Card/DeleteCard";
