...
function SearchCardReaderDataTableInitial() {
    ...
    const rowCallback = function (row, data) {
        (data.cr_mode == "連線") ? $('td:eq(4)', row).html("<b class='text-success'>連線</b>") : $('td:eq(4)', row).html("<b class='text-danger'>離線</b>");
        (data.cr_card_switch == "開啟") ? $('td:eq(5)', row).html("<b class='text-success'>開啟</b>") : $('td:eq(5)', row).html("<b class='text-danger'>關閉</b>");
        (data.cr_status == "線上") ? $('td:eq(6)', row).html("<b class='text-success'>線上</b>") : $('td:eq(6)', row).html("<b class='text-danger'>離線</b>");
    }
